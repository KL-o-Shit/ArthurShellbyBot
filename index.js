...
const config = {
    A187: '📍Arthur Shellby Bot📍',
    instagram: 'OFF',
    nomer: 'wa.me/5519981623441',
    aktif: '24 JAM',
   ...
   switch (prefix) {
      ...
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[ESPERE UM POUCO, ESTOU PROCESSANDO...', MessageType.text)
       ...
       case 'ytmp4':
           ytdl('mp4', value)
               .then(data => {
                   const { judul, size, hasil: link } = data
                   let hasil = `OLA AQUI ESTA O LINK DE DOWNLOAD DA MSC\nCLIQUE NO LINK ABAIXO:\nMUSICA: ${judul}\n\nTamanho Do audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[CALMA FIH DE CRISTO...', MessageType.text)
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'twt':
           tweetdl(value)
               .then(data => {
                    ...
                    client.sendMessage(id, '[CALMA FIH...', MessageType.text)
                    client.sendMessage(id, hasil ,MessageType.text)
               })
               .catch(err => {
                   console.log(err)
